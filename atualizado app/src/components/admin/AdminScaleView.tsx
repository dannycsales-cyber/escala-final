...
import { ChurchEvent, Assignment, Volunteer, Ministry, MINISTRY_REQUIREMENTS, getMinistryRequirement } from '../../types';
                      ...
                      <div className="p-8 bg-black/40 rounded-[2.5rem] border border-white/5">
                        ...
                        <div className="flex items-end justify-between mb-4">
                           <p className="text-4xl font-serif font-black text-white">
                              {assignments.filter(a => a.eventId === selectedEvent.id).length} / {selectedEvent.teamsNeeded.reduce((acc, min) => acc + getMinistryRequirement(min, selectedEvent.date), 0)}
                           </p>
                        ...
                        <div className="w-full h-2 bg-white/5 rounded-full overflow-hidden mb-8">
                           <motion.div 
                             initial={{ width: 0 }}
                             animate={{ width: `${(assignments.filter(a => a.eventId === selectedEvent.id).length / selectedEvent.teamsNeeded.reduce((acc, min) => acc + getMinistryRequirement(min, selectedEvent.date), 0)) * 100}%` }}
                       ...
                       <div className="flex items-center justify-between">
                         <h4 className="text-[10px] font-black text-gold uppercase tracking-[0.3em]">Gestão por Equipe</h4>
                         <span className="text-[9px] font-black text-white/20 uppercase tracking-widest italic">Visão Geral</span>
                       </div>
                       <div className="space-y-3 max-h-[500px] overflow-y-auto no-scrollbar pr-2">
                          {selectedEvent.teamsNeeded.map(min => {
                            const teamAssigned = assignments.filter(a => a.eventId === selectedEvent.id && a.ministry === min).length;
                            const required = getMinistryRequirement(min, selectedEvent.date);
                            const isFull = teamAssigned >= required;
                            const slotsLeft = Math.max(0, required - teamAssigned);
                            const isStrategic = ['KIDS', 'COZINHA', 'MÍDIA'].includes(min);

                            return (
                              <div key={min} className={cn(
                                "flex flex-col p-4 rounded-[2rem] border transition-all gap-4",
                                isFull 
                                 ...
                                 <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-3">
                                       ...
                                       <div>
                                         <p className={cn(
                                           ...
                                           isStrategic && !isFull ? "text-gold" : "text-white/80"
                                         )}>{min}</p>
                                         <div className="flex items-center gap-2 mt-1">
                                           {isFull ? (
                                             <div className="flex items-center gap-1">
                                               <CheckCircle2 className="w-2.5 h-2.5 text-green-500" />
                                               <span className="text-[7px] font-black uppercase text-green-500 tracking-widest">Equipe Completa</span>
                                             </div>
                                           ) : (
                                             <div className="flex items-center gap-1">
                                               <div className="w-1 h-1 rounded-full bg-red-500 animate-pulse" />
                                               <span className="text-[7px] font-black uppercase text-red-500/80 tracking-widest">
                                    ...
                                    <div className="flex items-center gap-2 px-3 py-1.5 bg-black/40 rounded-full border border-white/5">
                                       <span className={cn("text-[10px] font-black", isFull ? "text-green-500" : "text-white")}>
                                         {teamAssigned} / {required}
                                       </span>
                                    </div>
                                 </div>
                                 <div className="w-full h-[3px] bg-white/5 rounded-full overflow-hidden">
                                    <div 
                                      ...
                                      style={{ width: `${(teamAssigned/required)*100}%` }}
                                    />
                                 </div>
                              </div>
                            );
