...
export default function Home({ 
  ...
  return (
    <div className="max-w-md mx-auto space-y-8 pb-10">
      ...
      <div className="text-center pt-8 sm:pt-14 pb-8 sm:pb-12 space-y-8 sm:space-y-10 flex flex-col items-center">
        <motion.div 
        ...
        >
          ...
          <div className="w-[140px] h-[140px] sm:w-[180px] sm:h-[180px] relative flex items-center justify-center">
        ...
        <div className="space-y-4 sm:space-y-6">
          ...
          >
            <h2 className="text-xl sm:text-4xl md:text-6xl text-white font-serif font-light uppercase tracking-[0.3em] sm:tracking-[0.6em] ml-[0.3em] sm:ml-[0.6em] drop-shadow-[0_0_30px_rgba(212,175,55,0.6)]">
              SERVINDO AO <span className="text-gold italic font-medium">REINO</span>
            </h2>
            
            <div className="flex items-center gap-3 sm:gap-8 justify-center mt-2 sm:mt-6">
              <div className="hidden sm:block w-12 sm:w-16 h-[1px] bg-gold shadow-[0_0_8px_rgba(212,175,55,0.5)]" />
              <div className="flex items-center gap-3 sm:gap-8 px-2 sm:px-4">
                {['Propósito', 'Excelência', 'Reino'].map((t, i, a) => (
                  <React.Fragment key={t}>
                    <span className="text-[9px] sm:text-[14px] font-bold uppercase tracking-[0.2em] sm:tracking-[0.4em] text-white brightness-110 drop-shadow-[0_0_10px_rgba(212,175,55,0.3)]">{t}</span>
                    {i < a.length - 1 && <span className="text-gold opacity-70 text-[7px] sm:text-[10px]">◆</span>}
                  </React.Fragment>
                ))}
              </div>
              <div className="hidden sm:block w-12 sm:w-16 h-[1px] bg-gold shadow-[0_0_8px_rgba(212,175,55,0.5)]" />
          ...
          <motion.div 
            ...
            className="pt-4 sm:pt-10 border-t border-white/[0.08] w-full"
          >
            <h3 className="text-xl sm:text-4xl font-serif text-white tracking-tight">Olá, <span className="text-gold italic font-medium drop-shadow-[0_0_15px_rgba(212,175,55,0.4)]">{currentUser.name.split(' ')[0]}</span></h3>
            <p className="text-white/60 text-[9px] sm:text-sm uppercase tracking-[0.2em] sm:tracking-[0.4em] font-black mt-2 sm:mt-4">Membro do Ministério de {currentUser.primaryRole}</p>
          </motion.div>
          ...
          >
            ...
            <div className="p-6 sm:p-8 space-y-6 sm:space-y-8">
              <div className="space-y-1">
                <p className="text-[9px] text-white/30 uppercase font-black tracking-[0.3em]">Ministério</p>
                <div className="flex items-center gap-3">
                  <Star className="w-5 h-5 sm:w-6 sm:h-6 text-gold" />
                  <p className="text-xl sm:text-2xl font-serif text-white uppercase tracking-tight">{nextAssignment.ministry}</p>
                </div>
              </div>

              <div className="grid grid-cols-2 gap-4 sm:gap-8">
                <div className="space-y-1">
                  <p className="text-[9px] text-white/30 uppercase font-black tracking-[0.2em]">Data</p>
                  <p className="text-xs sm:text-sm font-black text-white uppercase tracking-wider">{format(parseISO(nextAssignment.event!.date), "dd 'de' MMMM", { locale: ptBR })}</p>
                </div>
                <div className="space-y-1">
                  <p className="text-[9px] text-white/30 uppercase font-black tracking-[0.2em]">Chegada</p>
                  <div className="flex items-center gap-2">
                    <Clock className="w-3 h-3 text-gold" />
                    <p className="text-xs sm:text-sm font-black text-white uppercase tracking-wider">{nextAssignment.event?.arrivalTime}</p>
                  </div>
                </div>
              </div>
